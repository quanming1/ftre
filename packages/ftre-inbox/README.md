...
- `next-step` 只按 Session 排队，不绑定某一次 Run；由下一次 Agent reasoning 边界交付；
- `followup`、`steer`、`inject` 三种写入语义；
- JSON 原子持久化、幂等 admission、`peek → Hook → claim` 和旧 `mailbox.pending` 一次性迁移；
- 队列编辑/删除/提升和 `session/queue` 权威投影；不启动自主调度循环。
...
运行状态机；交付失败由 AgentService 返回终态，Inbox 不自动回退或重投。
...
Plugin 协作，通过 `AgentService.run(InboundMessage)` 交付已 claim 输入。`agent/before-reasoning`
消费 `next-step`，`agent/after-run(status=completed)` 触发一条 `next-turn`；异常结束
不会自动消费队列。
