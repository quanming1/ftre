...
| 2026-08-17 | 修复：turn 结束后的 70% 预压缩检查此前仅在队列还有等待消息（peek 非空）时执行——空闲会话的压缩被推迟到下一条消息 before_claim 的 80% 强制水位才触发，客户端「压缩中」气泡直到用户再发消息才出现、新消息被迫排队等压缩。改为 turn 结束后无条件检查 after_turn 水位，队列空也在收尾时预压缩 | 用户反馈：压缩气泡有时不出现，发新消息才发现压缩中、消息在队列里 |
