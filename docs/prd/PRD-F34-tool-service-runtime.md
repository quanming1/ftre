...
| 2026-08-28 | 修复同步 ToolDefinition 直接运行在 Gateway 事件循环的问题：同步 callable 统一通过 `asyncio.to_thread` 执行，异步 callable 保持事件循环内 await；新增事件循环不阻塞回归测试 | bash 等同步 subprocess 工具遇到慢路径（如 OneDrive/网络目录）会阻塞全部 HTTP/WS 响应 |
| 2026-08-28 | 收敛 Tool Hook 与 Tool Service 的 `ToolExecutionResult` 为 `ftre_agent.tool` 单一类型，删除 Hooks 中重复 class；新增透明 `tool/after` 透传身份回归测试 | 透明审计 Hook 返回同名但不同身份的类型，导致每次工具执行在完成后被错误判为 Hook 协议错误 |
