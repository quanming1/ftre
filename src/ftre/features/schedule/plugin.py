...
async def apply(ctx: PluginContext, config=None):
    ...
    ctx.effect(service.close, label="schedule:close")
    ...
    if channel_disposer is not None and callable(getattr(ctx.sessions, "create_session", None)):
        ...
        scheduler.start()
        ctx.effect(scheduler.stop, label="schedule:scheduler")
