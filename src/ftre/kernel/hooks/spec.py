"""重新导出 Agent Core 提供的 Hook 契约类型。

``ftre`` 不在 Kernel 里复制一份 ``HookSpec`` 或 Hook 模式枚举。真正的契约由
``ftre-agent-core`` 持有，Core 和 Gateway 因而使用同一套类型与校验规则；本文件
...
from ftre_agent_core.hooks import HookFailurePolicy, HookMode, HookScope, HookSpec
