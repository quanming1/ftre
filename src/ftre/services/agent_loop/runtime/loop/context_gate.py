...
class ContextGate:
    ...
    async def before_claim(
        ...
        needed = await self._compaction.should_compact(
            session_id,
            channel_id,
            config,
            threshold=threshold,
    ...
    async def compact(
        ...
        still_over = await self._compaction.should_compact(
            session_id, channel_id, config, threshold=hard_threshold
        )
        if not still_over:
            return ContextDecision("pass")
        try:
            await self._compaction.compress_fast(session_id, channel_id, config=config)
        ...
        if await self._compaction.should_compact(
            session_id, channel_id, config, threshold=hard_threshold
        ):
    ...
    async def cancel(self, session_id: str) -> bool:
        ...
        return await self._compaction.cancel_compact(session_id)
