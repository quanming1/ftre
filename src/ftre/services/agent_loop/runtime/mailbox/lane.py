...
class SessionLane:
    ...
    async def submit(self, inbound: BusMessage) -> AdmissionResult:
        ...
        async with self._admission_lock:
            ...
            if admission.created:
                # 读取刚刚提交的只读快照以获得真实 sequence/content；worker 尚未
                # 启动，因此 inserted 观察与持久 mutation 仍是一一对应的。
                snapshot = await self._mailbox.snapshot(self.session_id)
                inserted = next(
                    (
                        pending
                        for pending in snapshot.pending
                        if pending.request_id == admission.request_id
                    ),
                    None,
                )
                if inserted is not None:
                    await self._emit_agent_observation(
                        AGENT_INBOX_INSERTED_SPEC, inserted
                    )
        ...
        async with self._admission_lock:
            ...
            if request_id:
                await self._completion.complete(
                    self.session_id, request_id, outcome
                )
        ...
        try:
            while not self._closed:
                ...
                if pre_step_signal.is_set():
                    ...
                    return
                if isinstance(pre_step, RejectStep):
                    if pre_step.disposition == "keep":
                        self._operation = BlockedOperation(pre_step.reason)
                        await self._mailbox.advance_revision(self.session_id)
                        await self._publish_snapshot(self.session_id)
                        return
                    discarded = await self._mailbox.cancel_pending(
                        self.session_id, item.request_id
                    )
                    if discarded is not None:
                        await self._emit_agent_observation(
                            ...
                            discarded,
                            turn_id=turn_id,
                            reason=pre_step.reason,
                        )
                        await self._completion.complete(
                            self.session_id,
                            item.request_id,
                            TurnOutcome(turn_id=turn_id, status="cancelled"),
                        )
                        await self._publish_snapshot(self.session_id)
                    continue
                if pre_step.candidate.request_id != item.request_id:
                    self._operation = BlockedOperation(
                        "agent/pre-step 返回了不匹配的 request_id"
                    )
                    await self._mailbox.advance_revision(self.session_id)
                    await self._publish_snapshot(self.session_id)
                    return
                ...
                self._operation = operation
                await self._emit_agent_observation(
                    AGENT_INBOX_CLAIMED_SPEC, item, turn_id=turn_id
                )
                await self._emit_agent_observation(
                    AGENT_SESSION_START_SPEC, item, turn_id=turn_id
                )
                await self._emit_agent_observation(
                    AGENT_STATUS_SPEC, item, turn_id=turn_id, reason="running"
                )
                execute_kwargs = {
                    "turn_id": turn_id,
                    "config": config,
            ...
            current_item = locals().get("item")
            await self._emit_agent_error(
                current_item.agent_id if isinstance(current_item, QueueItem) else "default",
                turn_id=(locals().get("turn_id") or ""),
                code="session_lane_error",
                message="SessionLane 内部错误；请求保留在队列中",
            )
    ...
    async def _pre_step(
        ...
        *,
        channel_id: str,
        config,
    ) -> EnterStep | RejectStep:
        """在 mailbox claim 前运行 Agent pre-step waterfall。"""
        if self._hooks is None:
            return EnterStep(PendingInput.from_queue_item(item))
        record = self._agent_registry.ensure(item.agent_id)
        payload = PreStepPayload(
            agent=AgentSubject(item.agent_id, record.identity),
            session_id=self.session_id,
            turn_id=turn_id,
            candidate=PendingInput.from_queue_item(item),
            cancellation=cancellation,
            channel_id=channel_id,
            config=config,
        )
    ...
    async def _emit_agent_observation(
        ...
        *,
        ...
        """Publish an Inbox/lifecycle observation without changing the mutation."""
        if self._hooks is None:
            return
        record = self._agent_registry.ensure(item.agent_id)
        payload_type = spec.payload_type
        if payload_type is AgentInboxPayload:
            payload = AgentInboxPayload(
                agent=AgentSubject(item.agent_id, record.identity),
                session_id=self.session_id,
                item=PendingInput.from_queue_item(item),
                turn_id=turn_id,
                reason=reason,
            )
        else:
            state = reason or spec.name.rsplit("/", 1)[-1]
        ...
        scope_context = self._hooks.context_for_scope(carrier)
        try:
            await self._hooks.dispatch(spec, payload, context=scope_context)
        except Exception:
            # Observation Hook 不能改变已经完成的 mailbox mutation；诊断已由
            # HookRuntime 记录，这里只保护 Lane 的数据面继续向前。
            logger.exception("[session-lane] agent observation failed hook=%s", spec.name)

    async def _emit_agent_error(
        self, agent_id: str, *, turn_id: str = "", code: str, message: str
    ) -> None:
        """Report a control/data-plane failure without masking the original failure."""
        if self._hooks is None:
            return
        record = self._agent_registry.ensure(agent_id)
        payload = AgentLifecyclePayload(
        ...
        carrier = self._agent_registry.scope_carrier(agent_id)
        try:
