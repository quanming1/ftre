...
这里故意不声明 LLM 失败/重试 Hook：实际 Agent Turn 的错误裁决由 Core 的
``llm/error`` 统一发布并消费。LlmService 只把失败作为 ``FinishChunk`` 返回，
避免再建立一条“返回值被忽略”的平行重试链。

``llm/stream`` 也直接重导出 Core 的 Spec。Core Runner 与 LlmService 内部调用
因此命中同一个 Hook 合约，Fallback Plugin 不会因为两份同名 Spec 触发冲突。
"""

from __future__ import annotations

from ftre_agent_core.hooks import LLM_STREAM, LLM_STREAM_SPEC
from ftre_agent_core.hooks import LLMStreamPayload as LlmStreamPayload
