...
| `sessions` | `session/service.py` | Session 身份/元信息唯一持久化入口；SessionLog 事件日志装配（append + write-behind + repair）、读侧 derive、session/event 帧转发 | AgentLoop、Inbox、Compaction、Workspace、Command |
...
Channel
  ...
  → SessionService（SessionLog.append 事件提交；write-behind 落盘 + 帧转发）
  → MessageBusService.publish_frame()（downstream_frame）
  → ChannelService / Channel
```

- `MessageBusService` 只负责传递，不保存会话业务状态。
- `SessionService` 拥有 session.json 元信息与 session.jsonl 事件日志（唯一事实源）；
  Msg 历史由读侧 `derive_messages` 从事件日志派生。独立 `InboxService` 负责 durable
