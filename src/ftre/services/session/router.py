...
def build_router(sessions, agents) -> APIRouter:
    ...
    async def cancel_queued_message(session_id: str, request_id: str):
        if await sessions.get_session(session_id) is None:
            raise HTTPException(status_code=404, detail=f"会话不存在: {session_id}")
        result = await agents.cancel_queued_message(session_id, request_id)
        if result is None:
            raise HTTPException(status_code=409, detail="消息不存在、已经执行，或者已经进入 processing 状态")
        return {"status": "cancelled", "session_id": session_id, "request_id": result.request_id}
    ...
    async def get_messages(
        ...
        mailbox = await agents.get_mailbox_snapshot(session_id)
        ...
        if limit_turns is not None and limit_turns > 0:
            ...
            return {"messages": messages, "has_more": has_more, "status": status, "mailbox": mailbox, "metadata": metadata}
        return {"messages": await sessions.get_messages_by_session(session_id), "status": status, "mailbox": mailbox, "metadata": metadata}
