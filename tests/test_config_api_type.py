...
class TestApiTypeFallback:
    def test_model_level_wins(self):
        cfg = build_llm_config(_provider_data(model_api_type="responses"), "p1", "test-model")
        assert cfg.api_type == "responses"

    def test_provider_level_fallback(self):
        cfg = build_llm_config(_provider_data(provider_api_type="responses"), "p1", "test-model")
        assert cfg.api_type == "responses"

    def test_default_completions(self):
        cfg = build_llm_config(_provider_data(), "p1", "test-model")
